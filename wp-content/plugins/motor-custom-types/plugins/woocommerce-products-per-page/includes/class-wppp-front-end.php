...
class WPPP_Front_End {
	...
	public function __construct() {
		...
		// Set dropdowns
		add_action( 'motor_catalog_top', array( $this, 'products_per_page_dropdown' ), 25 );
	...
	public function products_per_page_dropdown() {
		...
		?><form method="<?php echo esc_attr( $method ); ?>"<?php if (!empty($action)) : ?> action="<?php echo esc_url( $action ); ?>"<?php endif; ?> class="form-wppp-select products-per-page"><?php

			 do_action( 'wppp_before_dropdown' );

			?><p><?php esc_html_e('Per Page', 'motor-custom-types'); ?></p>
			<div class="dropdown-wrap">
				<p class="dropdown-title products-per-page-ttl"><?php if ($this->loop_shop_per_page() == '-1') echo esc_html__( 'All', 'motor-custom-types' ); else echo $this->loop_shop_per_page(); ?></p>
				<ul class="dropdown-list">
					<?php
					foreach( $products_per_page_options as $key => $value ) :

						?><li <?php if ( $value == $this->loop_shop_per_page() ) { echo ' class="active"'; } ?>><a data-value="<?php echo esc_attr( $value ); ?>" href="#"><?php
						$ppp_text = apply_filters( 'wppp_ppp_text', __( '%s', 'motor-custom-types' ), $value );
						esc_html( printf( $ppp_text, $value == -1 ? __( 'All', 'motor-custom-types' ) : $value ) );
						?></a></li><?php
					endforeach;
					?>
				</ul>
			...
			<select name="ppp" onchange="this.form.submit()" class="select wppp-select"><?php
				foreach( $products_per_page_options as $key => $value ) :
					...
					?><option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $this->loop_shop_per_page() ); ?>><?php
						$ppp_text = apply_filters( 'wppp_ppp_text', __( '%s', 'motor-custom-types' ), $value );
						esc_html( printf( $ppp_text, $value == -1 ? __( 'All', 'motor-custom-types' ) : $value ) ); // Set to 'All' when value is -1
					?></option><?php
				endforeach;
			?></select>
			<?php
