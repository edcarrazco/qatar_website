<?php
class MotorBadges_Widget extends WP_Widget {

    public function __construct() {
        parent::__construct(
            'motorbadges_widget',
            esc_html__( 'Motor Badges', 'motor-custom-types' ),
            array(
                'classname'   => 'motorbadges_widget',
                'description' => esc_html__( 'Badges List', 'motor-custom-types' )
            )
        );
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $terms_list = get_terms(array(
            'taxonomy'      => 'product_badges', // название таксономии с WP 4.5
            'orderby'       => 'name',
            'order'         => 'DESC',
            'hide_empty'    => true,
            'number'        => $posts_count,
        ...
        if (!empty($terms_list)) :
            ...
            <ul class="brands-list-sb">
                <?php foreach ($terms_list as $term) : ?>
                    <li>
                        <a href="<?php echo get_term_link($term->term_id, 'product_badges'); ?>"><?php echo $term->name; ?> <?php if (!empty($term->count)) echo '<span class="count">'.$term->count.'</span>'; ?></a>
                    </li>
                <?php endforeach; ?>
            </ul>
            <?php
...
/* Register the widget */
function motor_badges_widget_init () {
    register_widget( 'MotorBadges_Widget' );
}
add_action( 'widgets_init', 'motor_badges_widget_init');